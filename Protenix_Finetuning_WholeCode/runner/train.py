...
from protenix.data.kaggle_rna_dataset  import get_kaggle_rna_dataloaders
from protenix.data.dataset import get_msa_featurizer

# Disable WANDB's console output capture to reduce unnecessary logging
os.environ["WANDB_CONSOLE"] = "off"

# --- VRAM Logging Utility ---
def log_gpu_memory(step, device, phase=""):
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated(device) / (1024**3)  # Current allocated in GB
        reserved = torch.cuda.memory_reserved(device) / (1024**3)    # Current reserved in GB
        max_allocated = torch.cuda.max_memory_allocated(device) / (1024**3) # Peak allocated since last reset
        max_reserved = torch.cuda.max_memory_reserved(device) / (1024**3)   # Peak reserved since last reset
        logging.info(
            f"[GPU MEM {phase} Step {step}] Allocated: {allocated:.2f} GB, Reserved: {reserved:.2f} GB, "
...
class AF3Trainer(object):
    def __init__(self, configs):
        ...
        torch.cuda.reset_peak_memory_stats(self.device if self.use_cuda else None) 
        self.log_vram_flag = False
    ...
    def init_data(self):
        ...
        # )
        msa_featurizer_train = None
        msa_featurizer_val = None

        # Check if data.msa exists and is enabled
        if hasattr(self.configs.data, 'msa') and \
        self.configs.data.msa.enable and \
        hasattr(self.configs.data.msa, 'enable_rna_msa') and \
        self.configs.data.msa.enable_rna_msa:
            
            # Ensure train_sets is not empty and get the first dataset name
            if not self.configs.data.train_sets:
                logging.error("data.train_sets is empty. Cannot determine dataset_config_name for MSAFeaturizer.")
                # Decide how to handle: raise error, or proceed without MSA
            else:
                dataset_config_name = self.configs.data.train_sets[0] 
                logging.info(f"Initializing MSAFeaturizer for dataset: {dataset_config_name}")
                
                msa_featurizer_train = get_msa_featurizer(self.configs, dataset_name=dataset_config_name, stage="train")
                # For validation, it's common to use "test" stage settings for featurizers
                # Ensure test_sets is also configured if you want a distinct val featurizer based on test_sets[0]
                # If using the same dataset_config_name for val, it's fine.
                msa_featurizer_val = get_msa_featurizer(self.configs, dataset_name=dataset_config_name, stage="test")
        else:
            logging.info("MSA or RNA MSA is not enabled in the configuration. Proceeding without MSA.")
        
        self.train_dl, self.test_dls = get_kaggle_rna_dataloaders(
            self.configs, # get_kaggle_rna_dataloaders will access cfg.data.kaggle_rna3d
            msa_featurizer_train=msa_featurizer_train,
            msa_featurizer_val=msa_featurizer_val
        )
            ...
            for index, batch in enumerate(tqdm(test_dl)):
                ...
                with enable_amp:
                    ...
                    if torch.isnan(loss) or torch.isinf(loss):
                        self.print(f"WARNING: NaN/Inf loss for {ema_suffix}validation sample: {pid}. Loss: {loss.item()}. Skipping metrics for this sample.")
                        loss_is_nan = True
                            
                    # lDDT metrics
                    if not loss_is_nan: # Only compute further metrics if loss is valid
    ...
    def train_step(self, batch: dict):
        ...
        if self.log_vram_flag and (self.global_step % 5 == 0):
             log_gpu_memory(self.step, self.device, phase="Train-BeforeFwd")
        ...
        if self.log_vram_flag and (self.global_step % 5 == 0):
             log_gpu_memory(self.step, self.device, phase="Train-AfterFwdLoss")
        ...
        if self.log_vram_flag and (self.global_step % 5 == 0):
             log_gpu_memory(self.step, self.device, phase="Train-AfterBwd")
    ...
    def run(self):
        ...
        while True:
            if self.log_vram_flag:
                log_gpu_memory(self.step, self.device, phase="EpochStart/IterStart")
                torch.cuda.reset_peak_memory_stats(self.device if self.use_cuda else None)
                 
            for i, batch in enumerate(self.train_dl):
        
                if self.log_vram_flag and i==0: # Log after first batch loaded
                    log_gpu_memory(self.step, self.device, phase="Train-AfterBatchLoad")
                ...
                if step_need_log or is_last_step:
                    ...
                    if self.log_vram_flag and DIST_WRAPPER.rank == 0:
                        log_gpu_memory(self.step, self.device, phase="Train-AfterLog")
                        torch.cuda.reset_peak_memory_stats(self.device if self.use_cuda else None) 
                ...
                if step_need_eval or is_last_step:
                    self.evaluate()
                    if self.log_vram_flag and DIST_WRAPPER.rank == 0: # Log after eval and reset peak
                        log_gpu_memory(self.step, self.device, phase="Train-AfterEval")
                        torch.cuda.reset_peak_memory_stats(self.device if self.use_cuda else None)
