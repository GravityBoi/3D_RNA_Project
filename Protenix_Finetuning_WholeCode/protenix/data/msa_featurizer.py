...
# from protenix.data.msa_utils import process_single_sequence as process_single_sequence_util
...
class RNAMSAFeaturizer(BaseMSAFeaturizer):
    ...
    # def get_msa_path(
    #     self, db_name: str, sequence: str, pdb_id_entity_id: str, reduced: bool = True # db_name is now mostly ignored internally
    # ) -> str:
    #     # pdb_id_entity_id will be the target_id from your JSON file
    #     # The 'reduced' flag and its associated suffix logic might not be relevant
    #     # if you're not creating reduced versions of MSAs. Assuming suffix is empty for now.
    #     suffix = "_max_1w" if reduced else "" # Keep if you plan to use it, otherwise remove suffix logic.
    #                                         # For simplicity, let's assume suffix is not used for rna_align.sto.
    #     actual_pdb_id_entity_id = pdb_id_entity_id # This is what process_single_sequence will pass

    #     if self.indexing_method == "sequence":
    #         if sequence in self.seq_to_pdb_idx:
    #             # Use the first target_id associated with this sequence
    #             actual_pdb_id_entity_id = self.seq_to_pdb_idx[sequence][0]
    #         else:
    #             logger.warning(
    #                 f"Sequence not found in seq_to_pdb_idx. Cannot find MSA for a sequence starting with: {sequence[:30]}..."
    #             )
    #             # This will effectively make the path non-existent, which is handled later.
    #             return opjoin(self.rna_msa_dir, "non_existent_target/non_existent_db.sto")
        
    #     # MODIFIED: Construct path to the single 'rna_align.sto' file
    #     # We ignore the input db_name as we have a single MSA file per target.
    #     # actual_pdb_id_entity_id is the target_id.
    #     # The {suffix} part for rna_msa_dir might need adjustment based on your setup.
    #     # If rna_msa_dir already points to the directory containing target_id subfolders:
    #     # e.g. self.rna_msa_dir = "/path/to/RNA_MSA_Stockholm" (no suffix)
    #     # then path becomes /path/to/RNA_MSA_Stockholm/target_id/rna_align.sto
        
    #     # Original: rel_path = f"{actual_pdb_id_entity_id}/{db_name}.sto"
    #     # Original: msa_dir_path = opjoin(f"{self.rna_msa_dir}{suffix}", rel_path)
        
    #     # Simplified path construction for single MSA file:
    #     msa_file_path = opjoin(self.rna_msa_dir, actual_pdb_id_entity_id, "rna_align.sto")
    #     # If you use the suffix logic for the main rna_msa_dir:
    #     # msa_file_path = opjoin(f"{self.rna_msa_dir}{suffix}", actual_pdb_id_entity_id, "rna_align.sto")
        
    #     logger.debug(f"RNA MSA path for target {actual_pdb_id_entity_id}, sequence (truncated): {sequence[:30]}... -> {msa_file_path}")
    #     return msa_file_path
    
    def get_msa_path(self, db_name: str, sequence: str, pdb_id_entity_id: str, reduced: bool = True) -> str:
        # pdb_id_entity_id is the f"{sample_name_from_dataset}_{entity_id_from_atom_array}"
        # sequence is the original_full_sequence_from_input

        # DEBUG_FEATURIZE = False
        candidate_target_ids_for_dir = []

        if self.indexing_method == "sequence":
            if sequence in self.seq_to_pdb_idx:
                # self.seq_to_pdb_idx[sequence] is a list of target_ids, e.g., ["7P7U_D", "7P7Q_D", "7NHK_D"]
                candidate_target_ids_for_dir = self.seq_to_pdb_idx[sequence]
            else:
                # logger.warning(
                #     f"Sequence for {pdb_id_entity_id} not found in seq_to_pdb_idx. Cannot find MSA. Seq: {sequence[:30]}..."
                # )
                return opjoin(self.rna_msa_dir, "non_existent_target/non_existent_db.sto")
        elif self.indexing_method == "pdb_id_entity_id": # Should not be hit with your config
            candidate_target_ids_for_dir = [pdb_id_entity_id]
        else: # Should not be hit
            candidate_target_ids_for_dir = [pdb_id_entity_id.split('_')[0]] # Fallback attempt

        if not candidate_target_ids_for_dir: # If list is empty for some reason
            # logger.warning(f"No candidate target_ids found for MSA directory for {pdb_id_entity_id}. Seq: {sequence[:30]}...")
            return opjoin(self.rna_msa_dir, "non_existent_target/non_existent_db.sto")

        # Try each candidate target_id from the JSON map until a .sto file is found
        for target_id_for_dir in candidate_target_ids_for_dir:
            # Assuming `reduced` flag and `suffix` logic is not critical for your rna_align.sto files
            # suffix = "_max_1w" if reduced else ""
            # msa_file_path = opjoin(f"{self.rna_msa_dir}{suffix}", target_id_for_dir, "rna_align.sto")
            msa_file_path = opjoin(self.rna_msa_dir, target_id_for_dir, "rna_align.sto")
            
            # if DEBUG_FEATURIZE: # Assuming DEBUG_FEATURIZE is accessible or use logger.debug
            #     logger.info(f"Trying MSA path for {pdb_id_entity_id} (seq starting {sequence[:15]}...): {msa_file_path}")

            if opexists(msa_file_path) and msa_file_path.endswith(".sto"):
                # if DEBUG_FEATURIZE: logger.info(f"Found MSA at: {msa_file_path}")
                return msa_file_path
        
        # If no file found after trying all candidates
        # logger.warning(
        #     f"No MSA file found for {pdb_id_entity_id} (seq starting {sequence[:15]}...) after trying candidates: {candidate_target_ids_for_dir}. Looked in {self.rna_msa_dir}."
        # )
        return opjoin(self.rna_msa_dir, "non_existent_target/non_existent_db.sto") # Fallback if none found
    ...
    def process_single_sequence(
        self,
        pdb_name: str, # This is typically "{pdb_id}_{entity_id}" or just "target_id"
        sequence: str,
        pdb_id: str,   # The PDB ID part if pdb_name is composite, or can be same as pdb_name
        is_homomer_or_monomer: bool, # Should be True for RNA in this context
    ) -> dict[str, np.ndarray]:
        
        raw_msa_paths, seq_limits = [], []
        
        # Let's assume reduced=False, or handle based on config if needed.
        # Use pdb_name as the identifier for locating the MSA directory.
        msa_file_path = self.get_msa_path(db_name="ignored_db_name", sequence=sequence, pdb_id_entity_id=pdb_name, reduced=False)
        
        if opexists(msa_file_path) and msa_file_path.endswith(".sto"):
            raw_msa_paths.append(msa_file_path)
            # Use a generic limit or a configurable one for "rna_align.sto"
            # Defaulting to a high limit (e.g., rfam's default) or make this configurable.
            # Max size is also controlled by self.max_size and crop_chains later.
            limit_for_rna_align = self.seq_limits.get("rna_align_sto", SEQ_LIMITS.get("rfam", 10000))
            seq_limits.append(limit_for_rna_align)
        # else:
        #     logger.warning(f"RNA MSA file not found or not .sto for {pdb_name} (path: {msa_file_path}). Proceeding without MSA for this sequence.")
            # sequence_features will then be generated with dummy/empty MSA by process_single_sequence_util

        # This util function (from msa_utils) should be able to handle an empty raw_msa_paths list.
        sequence_features = process_single_sequence( # Renamed to avoid conflict if importing
            pdb_name=pdb_name, # This is target_id
            sequence=sequence,
            raw_msa_paths=raw_msa_paths,
            seq_limits=seq_limits,
            msa_entity_type="rna",
            msa_type="non_pairing", # RNA typically non-pairing
            # handle_empty="return_self" is default in load_and_process_msa
        )
        return sequence_features
...
class MSAFeaturizer:
    def __init__(
        ...
        self.prot_msa_featurizer = None
        if enable_prot_msa: # Check this flag
            if prot_msa_args.get("seq_to_pdb_idx_path") and \
               prot_msa_args.get("pdb_mmseqs_dir"):
                try:
                    self.prot_msa_featurizer = PROTMSAFeaturizer(**prot_msa_args)
                except Exception as e:
                    logger.error(f"Failed to initialize PROTMSAFeaturizer, disabling it. Error: {e}")
                    self.prot_msa_featurizer = None # Ensure it's None on failure
            else:
                logger.warning("Protein MSA essential paths not configured, PROTMSAFeaturizer will not be initialized.")
        else:
            logger.info("Protein MSA is explicitly disabled for MSAFeaturizer.")


        self.enable_rna_msa = enable_rna_msa
        self.rna_msa_featurizer = None # Initialize to None
        if self.enable_rna_msa:
            # Check if essential RNA MSA paths are properly configured
            if rna_msa_args.get("seq_to_pdb_idx_path") and \
               rna_msa_args.get("rna_msa_dir"):
                try:
                    self.rna_msa_featurizer = RNAMSAFeaturizer(**rna_msa_args)
                except Exception as e:
                    logger.error(f"Failed to initialize RNAMSAFeaturizer, disabling it. Error: {e}")
                    self.rna_msa_featurizer = None # Ensure it's None on failure
            else:
                logger.warning("RNA MSA essential paths not configured, RNAMSAFeaturizer will not be initialized.")
        else:
            logger.info("RNA MSA is disabled for MSAFeaturizer.")
    ...
    def __call__(
        ...
        prot_msa_feats = None
        if self.prot_msa_featurizer: # Check if it was initialized
            prot_msa_feats = self.prot_msa_featurizer.get_msa_features_for_assembly(
                bioassembly_dict=bioassembly_dict,
                entity_to_asym_id_int=entity_to_asym_id_int,
                selected_token_indices=selected_indices,
        ...
        rna_msa_feats = None
        if self.enable_rna_msa and self.rna_msa_featurizer: # Check if enabled AND initialized
            rna_msa_feats = self.rna_msa_featurizer.get_msa_features_for_assembly(
                bioassembly_dict=bioassembly_dict,
                entity_to_asym_id_int=entity_to_asym_id_int,
                selected_token_indices=selected_indices,
